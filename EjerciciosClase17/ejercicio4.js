...
function transformarYAnalizarNombres(entrada) {
  let lista = entrada.split(",").map(n => n.trim()); // 1

  let existeJuan = lista.includes("Juan");           // 2

  lista = lista.map(n => n === "Maria" ? "Ana" : n); // 3

  let indicePedro = lista.indexOf("Pedro");          // 4

  let ordenados = lista.sort().join(".");            // 5

  return { existeJuan, indicePedro, ordenados };
}

let nombres = prompt("Ingresá los nombres separados por comas: ");
